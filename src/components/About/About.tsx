"use client";

import Image from "next/image";
import { Typewriter } from "react-simple-typewriter";
import img from "../../../public/images/mojnumiah.png";

const About = () => {
  return (
    <section id="about" className="py-20 bg-gray-50">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h1 className="text-4xl md:text-5xl font-bold text-gray-800 mb-4">
            Md. Mojnu Miah
          </h1>
          <h5 className="text-xl md:text-2xl text-purple-600 font-semibold">
            <Typewriter
              words={[
                "Frontend Engineer",
                "React Specialist",
                "Next.js Expert",
              ]}
              loop={0}
              cursor
              cursorStyle="_"
              typeSpeed={70}
              deleteSpeed={50}
              delaySpeed={1000}
            />
          </h5>
        </div>

        <div className="flex flex-col md:flex-row items-center gap-12">
          <div className="md:w-1/2">
            <div className="summary-list">
              <ul className="space-y-4 text-gray-700">
                <li className="flex items-start">
                  <span className="text-purple-600 font-semibold mr-2">•</span>
                  <span>
                    <span className="font-semibold text-purple-600">
                      Expertise:
                    </span>{" "}
                    React.js, Next.js, Redux Toolkit, TypeScript
                  </span>
                </li>
                <li className="flex items-start">
                  <span className="text-purple-600 font-semibold mr-2">•</span>
                  <span>
                    <span className="font-semibold text-purple-600">
                      Technologies:
                    </span>{" "}
                    JavaScript (ES6+), Tailwind CSS, Material UI, React Query,
                    SWR
                  </span>
                </li>
                <li className="flex items-start">
                  <span className="text-purple-600 font-semibold mr-2">•</span>
                  <span>
                    <span className="font-semibold text-purple-600">
                      Tools:
                    </span>{" "}
                    Vite, Webpack, ESLint, Prettier, Git, GitHub Actions
                  </span>
                </li>
                <li className="flex items-start">
                  <span className="text-purple-600 font-semibold mr-2">•</span>
                  <span>
                    <span className="font-semibold text-purple-600">
                      Location:
                    </span>{" "}
                    Dhaka, Bangladesh
                  </span>
                </li>
              </ul>
            </div>

            <div className="mt-8">
              <blockquote className="text-lg italic text-gray-600 border-l-4 border-purple-600 pl-4 py-2">
                Results-driven Frontend Engineer with 3+ years of experience
                building scalable, high-performance web applications. Passionate
                about performance, code quality, and continuous learning.
              </blockquote>
            </div>

            <div className="mt-8 flex space-x-4">
              <a
                href="https://www.linkedin.com/in/mojnu0/"
                target="_blank"
                rel="noopener noreferrer"
                className="text-purple-600 hover:text-purple-800 transition-colors"
              >
                <svg
                  className="w-8 h-8"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                >
                  <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
                </svg>
              </a>
              <a
                href="https://github.com/codedByMojnu"
                target="_blank"
                rel="noopener noreferrer"
                className="text-purple-600 hover:text-purple-800 transition-colors"
              >
                <svg
                  className="w-8 h-8"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                >
                  <path
                    fillRule="evenodd"
                    d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z"
                    clipRule="evenodd"
                  />
                </svg>
              </a>
              <a
                href="https://www.facebook.com/mojnu0"
                target="_blank"
                rel="noopener noreferrer"
                className="text-purple-600 hover:text-purple-800 transition-colors"
              >
                <svg
                  className="w-8 h-8"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                >
                  <path
                    fillRule="evenodd"
                    d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z"
                    clipRule="evenodd"
                  />
                </svg>
              </a>
            </div>
          </div>

          <div className="md:w-1/2 flex justify-center">
            <div className="relative">
              <div className="w-64 h-64 md:w-80 md:h-80 rounded-full overflow-hidden border-4 border-purple-600">
                <Image
                  src={img}
                  alt="Md. Mojnu Miah"
                  layout="fill"
                  objectFit="cover"
                  className="rotate-img"
                />
              </div>
            </div>
          </div>
        </div>

        <div className="text-center mt-16">
          <h6 className="text-xl font-semibold text-purple-600">
            <Typewriter
              words={[
                "Top Frontend Developer in Bangladesh",
                "Building Scalable Web Applications",
              ]}
              loop={0}
              cursor
              cursorStyle="_"
              typeSpeed={70}
              deleteSpeed={50}
              delaySpeed={1000}
            />
          </h6>
          <div className="mt-8">
            <a
              href="#contact"
              className="inline-block bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 px-6 rounded-full transition-colors duration-300"
            >
              Want to discuss opportunities? Contact me &gt;&gt;
            </a>
          </div>
        </div>
      </div>
    </section>
  );
};

export default About;
